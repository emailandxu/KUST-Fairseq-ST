...
class MultilingualTranslationTask(FairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--is-shared-encoder', default=False, action='store_true')
    ...
    def setup_task(cls, args, **kwargs):
        ...
        srclangs = list({lang_pair.split('-')[0] for lang_pair in args.lang_pairs })
        langs = list({x for lang_pair in args.lang_pairs for x in lang_pair.split('-')})

        # load dictionaries
        dicts = OrderedDict()
        for lang in langs:
            if lang in srclangs and args.audio_input:
                dicts[lang] = AudioDictionary.load(os.path.join(args.data, 'dict.{}.txt'.format(lang)))
            else:
    ...
    def load_dataset(self, split, **kwargs):
        ...
        for lang_pair in lang_pairs:
            ...
            if self.args.audio_input:
                ...
                tgt_datasets[lang_pair] = indexed_dataset(prefix + tgt, self.dicts[tgt], cached=cached)
            else:
