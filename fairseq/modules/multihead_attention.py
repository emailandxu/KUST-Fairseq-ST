...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,**kwargs):
        super().__init__()
...
def gaussian(matrix, batch_size=1, variance=1e-6,**kwargs):
    num = (matrix * matrix).unsqueeze(0)
    denom = (2 * variance * variance).unsqueeze(1).unsqueeze(2).repeat(batch_size, 1, 1)
    return  num / denom

def log(matrix, batch_size=1, **kwargs):
    return torch.max(torch.zeros_like(matrix), torch.log(matrix))

#layer log
def llog(matrix,batch_size=1,layer_number=1,**kwargs):
    #distance matrix plus 2^layer_number
    return torch.max(torch.zeros_like(matrix), torch.log(matrix + 2**layer_number))
...
class LocalMultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, penalty='log', layer_number=1, **kwargs):
        super().__init__()
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.dropout = dropout
    ...
    def forward(self, query, key, value, mask_future_timesteps=False,
        ...
        layer_number = self.layer_number if hasattr(self,"layer_number") else None
        local_mask = self.penalty(pos_diff, bsz,
                                  variance=variance,
