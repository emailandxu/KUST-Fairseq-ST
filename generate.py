    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for sample_id, src_tokens, target_tokens, hypos in translations:
            ...
            if not args.quiet:
                ...
                if has_target:
                    if args.char_tokenized:
                        print('T-{}\t{}'.format(sample_id, target_str.replace(" ","").replace("|"," ")))
                    else:
            ...
            for i, hypo in enumerate(hypos[:min(len(hypos), args.nbest)]):
                ...
                if not args.quiet:
                    if args.char_tokenized:
                        print('H-{}\t{}\t{}'.format(sample_id, hypo['score'], hypo_str.replace(" ","").replace("|"," ")))
                    else:
                        print('H-{}\t{}\t{}'.format(sample_id, hypo['score'], hypo_str))

                    # print("HT-{}\t{}\t{}".format(sample_id, hypo['score'], hypo['tokens'].int().cpu()))
                    # print('P-{}\t{}'.format(
                    #     sample_id,
                    #     ' '.join(map(
                ...
                if has_target and i == 0:
                    ...
                    if args.char_tokenized:
                        ...
                        tgt_tok_dic = data.Dictionary.load(f"{args.word_level_dict_path}/dict.{args.target_lang}.txt")
                        target_tokens = tokenizer.Tokenizer.tokenize(
                            target_str.replace(" ","").replace("|"," "), tgt_tok_dic, add_if_not_exist=True)
                        hypo_tokens = tokenizer.Tokenizer.tokenize(
                            hypo_str.replace(" ","").replace("|"," "), tgt_tok_dic, add_if_not_exist=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--char-tokenized', action='store_true',
                        help='indicate that input are using character level token')
    parser.add_argument("--word-level-dict-path")
